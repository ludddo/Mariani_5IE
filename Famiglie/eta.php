...
<html lang="it">
    ...
    <?php 
        ...
        function calcolaEta($data_nascita) {
            $data_nascita = DateTime::createFromFormat('d/m/Y', $data_nascita);
            if (!$data_nascita) {
                throw new Exception("Formato data non valido: $data_nascita");
            }
            $oggi = new DateTime();
        ...
        foreach ($db as $persona) {
            $eta = calcolaEta($persona['data_nascita']);
            if ($eta >= $eta_minima) {
                $persona['eta'] = $eta;
                $personeTrovate[] = $persona;
            }
        }
    ?>
    <body>
        <div class="container mt-4">
            ...
            <?php if (!empty($personeTrovate)): ?>
                <div class="row">
                    <?php foreach ($personeTrovate as $index => $personaTrovata): ?>
                        <div class="col-md-4 mb-3">
                            <div class="card" style="width: 18rem;">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="card-text">
                                        <strong>Età:</strong> <?php echo $personaTrovata['eta']; ?><br>
                                        <strong>Indirizzo:</strong> <?php echo $personaTrovata['indirizzo']; ?><br>
                                    </p>
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#dettagliModal<?php echo $index; ?>">
                                        Mostra Dettagli
                                    </button>
                                </div>
                            </div>
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="dettagliModal<?php echo $index; ?>">
            ...
            <?php else: ?>
                <div class="alert alert-danger" role="alert">
                    Nessuna persona trovata con l'età minima fornita.
                </div>
            <?php endif; ?>
        </div>
